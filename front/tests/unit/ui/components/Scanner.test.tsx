...
import { error, existing, Scanner, success } from '../../../../src/ui/components/Scanner'
...
describe('Scanner', () => {
  beforeEach(() => {
    mock(scanBook).mockResolvedValue(true)
  ...
  it('should buzz on scan error', async () => {
    mock(scanBook).mockRejectedValue(new Error())
    render(<Scanner refresh={jest.fn()} />)
    fireEvent.change(screen.getByPlaceholderText('ISBN'), { target: { value: 'isbn' } })
    fireEvent.submit(screen.getByRole('form'))
    await wait()
  ...
  it('should plop if book already exists', async () => {
    mock(scanBook).mockResolvedValue(false)
    render(<Scanner refresh={jest.fn()} />)
    fireEvent.change(screen.getByPlaceholderText('ISBN'), { target: { value: 'isbn' } })
    fireEvent.submit(screen.getByRole('form'))
    await wait()
