...
import { mock, mockBook } from '../../mocks'

jest.mock('../../../src/services/Axios')

describe('getBooks', () => {
  it('should get books', async () => {
    mock(Axios.get).mockResolvedValue({ data: 'books' })
    await getBooks()
...
describe('getBook', () => {
  it('should get book', async () => {
    mock(Axios.get).mockResolvedValue({ data: 'book' })
    await getBook('id')
...
describe('saveBook', () => {
  it('should update book', async () => {
    mock(Axios.put).mockResolvedValue({ data: 'book' })
...
describe('scanBook', () => {
  it('should create book', async () => {
    mock(Axios.post).mockResolvedValue({ status: 201 })
  ...
  it('should return false if book already existed', async () => {
    mock(Axios.post).mockResolvedValue({ status: 204 })
    const result = await scanBook('serial')
